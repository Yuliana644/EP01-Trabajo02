window.onload = function()
{
    ...
    */
    var validaSudoku = function(valor, id)
    {
        ...
        console.log(parteID);
        console.log(valor);
        /*aca se alinean ambos arreglos en un string para comparar, no se como sacar 
        el sudoku final o que el arreglo sudoku se actualice*/
        for(var fila = 0; fila < sudoku.length; fila++)
        {
            for(var col = 0; col < sudoku.length; col++)
            {
                for(var i = 0; i < sudoku.length; i++)
                {
                    ...
                    {
                        sudoku[fila][col][i][c]=valor
                        ss=sudoku[fila][col][i][c]
                        sa=solve[fila][col][i][c];
                        console.log(sa,ss);
                        if (ss==sa) 
                        {
                            var color = true;
                        ...
                        validar= document.getElementById("va");
                        validar.style.background = color? "green" : "red";
                    }
                }
            }
        }
    ...
    var comparaSudokuSolve = function(valor)
    {
        for(var fila = 0; fila < sudoku.length; fila++)
        {
            for(var col = 0; col < sudoku.length; col++)
            {
                for(var i = 0; i < sudoku.length; i++)
                {
                    for(var c = 0; c < sudoku.length; c++)
                    {
                        console.log(sudoku[fila][col][i][c], solve[fila][col][i][c]);
                    }
                }
            }
        }
