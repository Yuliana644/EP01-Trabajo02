...
//Metodo de comparacion y color  <-------------------------SOLUCION
    mathSquare.sort();
    var pasa = mathSquare.join();
    console.log(pasa);

   if(pasa.indexOf(valInput) >= 0)
   {
    var cor = true
   }
   else
   ...
   nom_div(respuesta.horizontal.div).style.background = cor? "green" : "red";
   nom_div(respuesta.vertical.div).style.background = cor? "green" : "red";  
